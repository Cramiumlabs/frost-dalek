...
mod dkg_benches {
    ...
    fn participant_new(c: &mut Criterion) {
        ...
        c.bench_function("Participant creation", move |b| {
            b.iter(|| Participant::new(&params, 1, &mut rng));
        });
    }
    ...
    fn round_one_3_out_of_5(c: &mut Criterion) {
        ...
        let (p2, _p2coeffs) = Participant::new(&params, 2, &mut rng);
        let (p3, _p3coeffs) = Participant::new(&params, 3, &mut rng);
        let (p4, _p4coeffs) = Participant::new(&params, 4, &mut rng);
        let (p5, _p5coeffs) = Participant::new(&params, 5, &mut rng);
